...
# =========================================================
# ADMIN COMMANDS (BOT PRIVATE CHAT)
# =========================================================
...
def admin_get_users(message):
    ...
    if not users_list:
        bot.send_message(message.chat.id, "📊 **Registered Users List:**\n\nKoi user registered nahi hai.", parse_mode="Markdown")
        return
    ...
    if len(user_list_text) > 4000:
        for x in range(0, len(user_list_text), 4000):
            bot.send_message(message.chat.id, user_list_text[x:x+4000], parse_mode="Markdown")
    else:
...
def admin_broadcast(message):
    ...
    if not broadcast_msg:
        bot.send_message(message.chat.id, "⚠️ **Format:** `/broadcast Aapka Message Yaha`", parse_mode="Markdown")
        return

    users_list = list(col_users.find({}))
    if not users_list:
        bot.send_message(message.chat.id, "❌ Broadcast karne ke liye koi registered user nahi mila.", parse_mode="Markdown")
        return
