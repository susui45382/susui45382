...
# =========================================================
# DATABASE LOAD & SAVE SYSTEM (FOR USER.JSON)
# =========================================================

def load_database():
    """Bot start hone par user.json se permanent data load karega"""
    default_db = {
        "used_utrs": [],
        "gmail_stock": [],
        "number_stock": [],
...
def save_database():
    """User data & dynamic stocks ko instant user.json me save karta hai"""
...
# Initializing global state from user.json
db = load_database()
...
def save_user_info(user_obj):
    """Har active user ka metadata user.json me sync karne ke liye"""
    if not user_obj:
        return
...
def record_purchase(user_id, utr, data_str):
    """User ke purchase order ko permanent store karta hai"""
...
def track_all_user_activities(bot_instance, update):
    """User click/text aate hi user.json update ho jayega"""
...
def check_bot_status(message):
    ...
    bot.send_message(
        ...
        f"📧 **Gmail Stock:** `{len(gmail_stock_list)}` \n"
        f"📞 **Number Stock:** `{len(number_stock_list)}` \n"
        f"👥 **FB Stock:** `{len(fb_stock_list)}` \n"
        f"🐦 **Twitter Stock:** `{len(twitter_stock_list)}` \n"
        f"📸 **Insta Stock:** `{len(insta_stock_list)}`", 
        parse_mode="Markdown"
    )

# =========================================================
# STOCK CAPTURE HANDLERS (SAVE TO USER.JSON)
# =========================================================
...
def handle_payment_screenshot(message):
    ...
    try:
        ...
        # Reserve UTR & Sync user.json Immediate Write
...
if __name__ == "__main__":
    print("Bot starting with full user.json persistence...")
